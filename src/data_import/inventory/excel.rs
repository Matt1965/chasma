...
use super::schema::{InventoryProfileImportRow, REQUIRED_COLUMNS};
use crate::data_import::error::{DataImportError, RowImportError};
use crate::data_import::schema::parse_enabled_cell;
use crate::world::InventoryAccessType;

pub const INVENTORY_PROFILES_SHEET_NAME: &str = "Inventory Profiles";
...
pub fn read_inventory_profile_rows(
    path: &std::path::Path,
) -> Result<Vec<Result<InventoryProfileImportRow, RowImportError>>, DataImportError> {
    use calamine::{Reader, Xlsx, XlsxError, open_workbook};

    let mut workbook: Xlsx<_> = open_workbook(path)
        .map_err(|err: XlsxError| DataImportError::WorkbookOpen(err.to_string()))?;
    let range = workbook
        .worksheet_range(INVENTORY_PROFILES_SHEET_NAME)
        .map_err(|_| DataImportError::SheetNotFound {
            sheet: INVENTORY_PROFILES_SHEET_NAME.to_string(),
        })?;
...
fn parse_row(
...
) -> Result<InventoryProfileImportRow, String> {
    ...
    let parse_optional_u32 = |column: &str| -> Result<Option<u32>, String> {
        let raw = text(column);
        let raw = raw.trim();
        if raw.is_empty() {
            Ok(None)
    ...
    let access_type = if columns.contains_key("Access Type") {
        InventoryAccessType::parse(&text("Access Type"))?
    } else {
    ...
    Ok(InventoryProfileImportRow {
        ...
        name: text("Name"),
        grid_width: parse_u8("Grid Width")?,
        grid_height: parse_u8("Grid Height")?,
