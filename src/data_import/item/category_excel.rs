...
use super::category_schema::{
    ItemCategoryImportRow, REQUIRED_COLUMNS as CATEGORY_REQUIRED_COLUMNS,
};
use crate::data_import::error::{DataImportError, RowImportError};
use crate::data_import::schema::parse_enabled_cell;

pub const ITEM_CATEGORIES_SHEET_NAME: &str = "Item Categories";
...
pub fn column_map_from_headers(
    ...
    for &required in CATEGORY_REQUIRED_COLUMNS {
...
pub fn read_item_category_rows(
    path: &std::path::Path,
) -> Result<Vec<Result<ItemCategoryImportRow, RowImportError>>, DataImportError> {
    use calamine::{Reader, Xlsx, XlsxError, open_workbook};

    let mut workbook: Xlsx<_> = open_workbook(path)
        .map_err(|err: XlsxError| DataImportError::WorkbookOpen(err.to_string()))?;
    let range = workbook
        .worksheet_range(ITEM_CATEGORIES_SHEET_NAME)
        .map_err(|_| DataImportError::SheetNotFound {
            sheet: ITEM_CATEGORIES_SHEET_NAME.to_string(),
        })?;
...
fn parse_row(
...
) -> Result<ItemCategoryImportRow, String> {
    ...
    let sort_priority = if columns.contains_key("Sort Priority") {
        let raw = text("Sort Priority");
        let raw = raw.trim();
        if raw.is_empty() {
            None
        } else {
            Some(
                raw.parse::<u32>()
                    .map_err(|_| format!("invalid Sort Priority `{raw}`"))?,
            )
        }
    } else {
        None
