//! Screen-ray picking for unit selection and terrain move commands (ADR-033 U8).
...
use crate::world::{UnitCatalog, UnitDefinition, UnitId, WorldData, WorldPosition};
...
/// Pick an authoritative grounded terrain position along `ray`.
pub fn pick_terrain_position_along_ray(
    ray: &Ray3d,
    world: &WorldData,
    ...
    terrain_click_to_world_position(ray, world, layout, vertical_scale)
        .map(|result| result.world_position)
}
...
fn ray_sphere_hit_distance(ray: &Ray3d, center: Vec3, radius: f32) -> Option<f32> {
...
mod tests {
    use super::*;
    use crate::world::{
        UnitDefinitionId, UnitRenderKey, UnitDefinition,
    };
