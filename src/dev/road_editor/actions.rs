...
use crate::world::{
    RoadDeformationStore, RoadNetwork, RoadTerrainRebuildQueue, WorldConfig, WorldData,
    affected_chunk_ids_for_network, detach_junction, junction_id_for_endpoint,
    load_road_network_from_world_package, queue_road_terrain_rebuilds,
    rebake_road_deformation_for_chunks, refresh_tee_branches_for_host, save_road_deformation_bake,
    save_road_network, sync_store_tiles_to_chunks, try_snap_endpoint, validate_road_network,
    DEFAULT_WORLD_PACKAGE_DIR,
};
...
pub fn handle_road_editor_buttons(
    ...
    mut road_store: ResMut<RoadDeformationStore>,
    ...
    for (interaction, button) in &buttons {
        ...
        match button {
            ...
            RoadEditorButton::Save => save_network(
                &mut editor,
                &network,
                &mut world,
                &mut road_store,
...
fn save_network(
    ...
    rebuild_queue: &mut RoadTerrainRebuildQueue,
    ...
    let baseline = editor.saved_baseline.clone();

    match save_road_network(network) {

        Ok(()) => {

            let old_chunks = affected_chunk_ids_for_network(&baseline, layout);

            let new_chunks = affected_chunk_ids_for_network(network, layout);

            let dirty: HashSet<_> = old_chunks.union(&new_chunks).copied().collect();
            ...
            queue_road_terrain_rebuilds(rebuild_queue, &dirty);

            if let Err(error) =

                save_road_deformation_bake(DEFAULT_WORLD_PACKAGE_DIR, network, road_store)

            {

                editor.status_message =

                    format!("Saved roads; deformation bake persist failed: {error}");
