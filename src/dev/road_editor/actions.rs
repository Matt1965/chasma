...
pub fn handle_road_editor_buttons(
    ...
    for (interaction, button) in &buttons {
        ...
        match button {
            ...
            RoadEditorButton::Cancel => editor.cancel_active(),
            RoadEditorButton::InsertPoint => {
                if editor.selected_road_id.is_none() {
                    editor.status_message = "Select a road before inserting a point".into();
                    continue;
                }
                editor.mode = RoadEditMode::InsertPoint;
                editor.status_message = "Click a road segment to insert a point".into();
            }
            RoadEditorButton::ExtendStart => start_extend(&mut editor, true),
            RoadEditorButton::ExtendEnd => start_extend(&mut editor, false),
...
fn start_extend(editor: &mut RoadEditorUiState, from_start: bool) {
    if editor.selected_road_id.is_none() {
        editor.status_message = "Select a road before extending".into();
        return;
    }
    ...
    editor.status_message = "Click terrain to add extension points, then Finish".into();
}
...
fn finish_active_operation(editor: &mut RoadEditorUiState, network: &mut RoadNetwork) {
    match editor.mode {
        ...
        RoadEditMode::ExtendStart | RoadEditMode::ExtendEnd => {
            editor.clear_draft_modes();
            editor.mark_dirty("Finished road extension");
        }
        _ => editor.status_message = "Nothing to finish".into(),
    }
}
...
fn cycle_selected_style(editor: &mut RoadEditorUiState, network: &mut RoadNetwork) {
    if editor.mode == RoadEditMode::Create {
        ...
        editor.mark_dirty("Updated draft road style");
        return;
    }
    let Some(road_id) = editor.selected_road_id.clone() else {
