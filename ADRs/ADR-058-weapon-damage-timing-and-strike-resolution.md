...
Combat strike timers advance in `step_all_combat_strikes`, called after
`step_all_combat_engagement` and before command resolve / movement:

1. `step_all_combat_strikes` (fixed delta)
2. `step_all_combat_engagement`
3. `resolve_pending_unit_orders`
4. `step_unit_movement`

Player simulation passes [`SIMULATION_TICK_SECONDS`] (30 Hz), not render
`Time::delta_secs()`. Pause / `step_once` gate at `SimulationControlState::begin_tick()`.
